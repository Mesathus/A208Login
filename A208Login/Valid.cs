        ...
        {            
            try
            {
                ...
                using (SqlConnection sqlcon = new SqlConnection(connectionstring))
                {
                    ...
                    SqlParameter passParam = new SqlParameter("@password", SqlDbType.NVarChar, 50);
                    SqlParameter IDParam = new SqlParameter("@studentKey", SqlDbType.NVarChar, 50);
                    SqlCommand cmd = new SqlCommand();
                    string insertname = "INSERT INTO dbo.Student (StudentKey, StudentID, FirstName, LastName) VALUES (@studentKey, @password, @fname, @lname);";                
                    sqlcon.Open();
                    fnameParam.Value = student.First;
                    lnameParam.Value = student.Last;
                    passParam.Value = Utility.Encrypt(student.Pass, false);
                    IDParam.Value = Utility.Encrypt((student.First[0] + student.Last).ToLower() + student.Pass, false);
